...
class MaterializePaginationTagLib {

    /**
     * Creates next/previous links to support pagination for the current controller while using MaterializeCSS<br/>
     *
     * &lt;g:materializePagination total="${Account.count()}" color="materializeCssColorClass" /&gt;<br/>
     *
     * @emptyTag
     *
     ...
     * @attr color REQUIRED Choose your desired color template.
     */
    def materializePagination = { Map attrsMap ->

        TypeConvertingMap attrs = (TypeConvertingMap)attrsMap
        def writer = out
        if (attrs.total == null) {
            throwTagError("Tag [materializePagination] is missing required attributes [total]")
        }
        ...
        def color = (String.valueOf(attrs.get('color')) ?: 'blue-grey')
        ...
        if (steps && laststep > firststep) {
            ...
            (beginstep..endstep).each { int i ->
                writer << '<li>'
                if (currentstep == i) {
                    writer << '<span class="btn '+color+' active">'+i.toString()+'</span>';
                }
                else {
